...
SELECT COALESCE("user".name, 'not defined') AS name,
       ...
       table_currency.name AS currency_name,
       table_currency.money * COALESCE(table_currency.rate_min, table_currency.rate_max) AS currency_in_usd
 ...
 GROUP BY balance.money,
          ...
          balance.currency_id,
          balance.user_id
 ORDER BY rate_min DESC, rate_max) AS table_currency
  LEFT JOIN "user"
    ON table_currency.user_id = "user".id
